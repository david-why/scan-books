...
ap.add_argument('-o', '--output', type=str, help='Output file name. "-" means standard input. Default: "-"', required=False, default='-', dest='outfile', metavar='FILE')
args = ap.parse_args()
...
frame = pd.read_sql_query('SELECT name, author, isbn FROM %s ORDER BY timestamp' % args.intable, conn)
if args.outfile == '-':
	print(frame.to_html(index=False))
else:
